...
| **NSE public endpoint access breaks** *(v2.0, MN-2)* | **High — most likely recurring operational failure** | Measured (Phase 1a F-6): a **User-Agent header suffices**, cookies are not required, but NSE applies **progressive throttling** under sustained load — slow-walking responses, then dropping connections. M01a isolates all access logic behind one module so a break is a single-file fix; L0 archive means a break stops new data but never destroys history; runbook §21.7 covers diagnosis. |
| **Bulk backfill throttled to a crawl** *(Phase 1a F-6)* | **Medium — schedule risk, not correctness** | A 2-yr backfill stalled at 5% under a 1s delay. Backfill must run unattended over hours with a 5–10s delay, using the resume capability. The nightly run (~3 files) is far below the threshold and is unaffected. SmartAPI (V9) may carry the equity side; F&O contract history has no substitute. |
